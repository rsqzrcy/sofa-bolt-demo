...
//    }
	...
	/**
	 * @see BaseRemoting#createInvokeFuture(RemotingCommand, InvokeContext)
	 */
	@Override
	protected InvokeFuture createInvokeFuture(RemotingCommand request, InvokeContext invokeContext) {
		ProtocolCode protocolCode = request.getProtocolCode();
		return new DefaultInvokeFuture(request.getId(), null, null, protocolCode, this.getCommandFactory(), invokeContext);
	}
	...
	protected InvokeFuture createInvokeFuture(Connection conn, RemotingCommand request, InvokeContext invokeContext, InvokeCallback invokeCallback) {
		ProtocolCode protocolCode = request.getProtocolCode();
		return new DefaultInvokeFuture(request.getId(), new RpcInvokeCallbackListener(RemotingUtil.parseRemoteAddress(conn.getChannel())), invokeCallback, protocolCode, this.getCommandFactory(), invokeContext);
	}
}
